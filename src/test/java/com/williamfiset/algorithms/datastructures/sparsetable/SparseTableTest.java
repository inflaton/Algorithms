...
public class SparseTableTest {
	...
	private void queryResultTest(long[] values, int l, int r, long actual, int index, SparseTable.Operation op) {
	...
	private void testAllOperations(long[] values) {
		...
		for (int i = 0; i < values.length; i++) {
			for (int j = i; j < values.length; j++) {
				queryResultTest(values, i, j, min_st.query(i, j), min_st.queryIndex(i, j), SparseTable.Operation.MIN);
				queryResultTest(values, i, j, max_st.query(i, j), max_st.queryIndex(i, j), SparseTable.Operation.MAX);
				queryResultTest(values, i, j, sum_st.query(i, j), -1 /* unused */, SparseTable.Operation.SUM);
				queryResultTest(values, i, j, mult_st.query(i, j), -1 /* unused */, SparseTable.Operation.MULT);
				queryResultTest(values, i, j, gcd_st.query(i, j), -1 /* unused */, SparseTable.Operation.GCD);
			}
		}
	}
