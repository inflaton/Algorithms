...
public class TarjanSccSolverAdjacencyList {
	...
	private void dfs(int at) {
		...
		for (int to : graph.get(at)) {
			 ...
			 * the following bit of code. From my experience this doesn't seem to matter if
			 * the output is placed in a separate output array, but this needs further
			 * investigation.
			 * 
			 * if (ids[to] == UNVISITED) { dfs(to); low[at] = min(low[at], low[to]); } if
			 * (visited[to]) { low[at] = min(low[at], ids[to]); }
			 */
