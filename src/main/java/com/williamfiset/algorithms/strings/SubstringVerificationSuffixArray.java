 ...
 *
 * <p>
 * Time complexity: O(nlogn) for suffix array construction and O(mlogn) time for individual queries
 * (where m is query string length). As noted below, depending on the length of the string (if it is
 * very large) it may be faster to use KMP or if you're doing a lot of queries on small strings then
 * Rabin-Karp in combination with a bloom filter.
 *
  ...
  public static class SuffixArray {
    ...
    public boolean contains(String substr) {
      ...
      while (lo <= hi) {
        ...
        if (suffix_len <= substr_len)
          suffix_str = new String(T, suffix_index, suffix_len);
        else
          suffix_str = new String(T, suffix_index, substr_len);
