...
public class CombinationsWithRepetition {
	...
	/**
	 * Computes all combinations of elements of 'r' elements which can be repeated
	 * at most 'k' times each.
	 *
	 * @param sequence  - The sequence containing all the elements we wish to take
	 *                  combinations from
	 ...
	 * @param k         - The maximum number of times each element is allowed to be
	 *                  picked
	 */
	private static void combinationsWithRepetition(int[] sequence, int[] usedCount, int at, int r, int k) {
		...
		if (at == N) {
			...
			// For this particular time at position 'at' try including it each of [0, k]
			// times
			for (int itemCount = 0; itemCount <= k; itemCount++) {

				// Try including this element itemCount number of times (this is possibly more
				// than once)
