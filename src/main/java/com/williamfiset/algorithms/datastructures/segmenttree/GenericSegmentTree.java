...
public class GenericSegmentTree {
	...
	public static enum RangeUpdateFn {
		// When a range update is issued, assign all the values in the range [l, r] to
		// be `x`
		ASSIGN,
		// When a range update is issued, add a value of `x` to all the elements in the
		// range [l, r]
		ADDITION,
		// When a range update is issued, multiply all elements in the range [l, r] by a
		// value of `x`
		MULTIPLICATION
	}
	...
	private Ruf lproductQueryMulUpdate = (b, tl, tr, d) -> safeMul(b, d); // safeMul(b, (long)(Math.pow(d, (tr - tl +
																			// 1))));
	...
	public GenericSegmentTree(long[] values, SegmentCombinationFn segmentCombinationFunction,
		...
		//
		// TODO(william): Investigate to reduce this space. There are only 2n-1
		// segments, so we should
		// be able to reduce the space, but may need to reorganize the tree/queries. One
		// idea is to use
		// the Eulerian tour structure of the tree to densely pack the segments.
		int N = 4 * n;

		t = new Long[N];
		// TODO(william): Change this to be of size n to reduce memory from O(4n) to
		// O(3n)
		lazy = new Long[N];
	...
	// Apply the lazy delta value to the current node and push it to the child
	// segments
	...
	// O(log(n))
	// *
	// * <p>Do this by performing a binary search to find the interval containing
	// the point, then
	// update
	// * the leaf segment with the new value, and re-compute all affected segment
	// values on the
	// * callback.
	// *
	// * @param i the index of the current segment in the tree
	...
	private static void productQueryMulUpdateExample() {
		// 0, 1, 2, 3
		long[] v = { 3, 2, 2, 1 };
		GenericSegmentTree st = new GenericSegmentTree(v, SegmentCombinationFn.PRODUCT, RangeUpdateFn.MULTIPLICATION);
	...
	private static void gcdQueryMulUpdateExample() {
		// 0, 1, 2, 3, 4
		long[] v = { 12, 24, 3, 4, -1 };
		GenericSegmentTree st = new GenericSegmentTree(v, SegmentCombinationFn.GCD, RangeUpdateFn.MULTIPLICATION);
	...
	private static void gcdQueryAssignUpdateExample() {
		...
		GenericSegmentTree st = new GenericSegmentTree(v, SegmentCombinationFn.GCD, RangeUpdateFn.ASSIGN);
	...
	private static void sumQuerySumUpdateExample() {
		// 0, 1, 2, 3, 4
		long[] v = { 2, 1, 3, 4, -1 };
		GenericSegmentTree st = new GenericSegmentTree(v, SegmentCombinationFn.SUM, RangeUpdateFn.ADDITION);
