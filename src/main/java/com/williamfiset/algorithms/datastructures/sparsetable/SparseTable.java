...
public class SparseTable {
	...
	// All functions must be associative, e.g: a * (b * c) = (a * b) * c for some
	// operation '*'
	...
	public int queryIndex(int l, int r) {
		...
		throw new UnsupportedOperationException("Operation type: " + op + " doesn't support index queries :/");
	}
	...
	//
	// Perform a cascading query which shrinks the left endpoint while summing over
	// all the intervals
	// which are powers of 2 between [l, r].
	//
	// WARNING: This method can easily produces values that overflow.
	//
	// NOTE: You can achieve a faster time complexity and use less memory with a
	// simple prefix sum
	// array. This method is here more as a proof of concept than for its
	// usefulness.
	...
	//
	// We can get O(1) query by finding the smallest power of 2 that fits within the
	// interval length
	// which we'll call k. Then we can query the intervals [l, l+k] and [r-k+1, r]
	// (which likely
	// overlap) and apply the function again. Some functions (like min and max)
	// don't care about
	// overlapping intervals so this trick works, but for a function like sum this
	// would return the
