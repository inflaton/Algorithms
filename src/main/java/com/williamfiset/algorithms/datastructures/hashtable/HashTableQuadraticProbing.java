 ...
 *
 * <p>
 * In this implementation we are using the following probing function: H(k, x) = h(k) + f(x) mod 2^n
 *
 * <p>
 * Where h(k) is the hash for the given key, f(x) = (x + x^2) / 2 and n is a natural number. We are
 * using this probing function because it is guaranteed to find an empty cell (i.e it generates all
