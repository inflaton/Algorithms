...
public class GeneralKDTree<T extends Comparable<T>> {
	...
	private T[] findMinRecursive(int dim, KDNode<T> curr, int axis) {
		...
		// Compare with the value in the current node and return the point with the
		// smallest value in
		// the given dimension
	...
	private T[] deleteRecursiveRoot() {
		...
		// Otherwise, get the minimum from the left subtree and make it the right
		// subtree
		else {
	...
	private class KDNode<E extends Comparable<E>> {
		...
		public KDNode(E[] coords) {
			...
			if (coords.length != k)
				throw new IllegalArgumentException("Error: Expected " + k + "dimensions, but given " + coords.length);
			point = coords;
			left = null;
			right = null;
		}
