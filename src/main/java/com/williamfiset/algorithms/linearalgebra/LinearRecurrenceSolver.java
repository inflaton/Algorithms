...
class LinearRecurrenceSolver {
	...
	/**
	 * Solve for the nth term in a linear recurrence of the following form f(n) = k
	 * + c_1*f(n-1) + c_2*f(n-2) + ... + c_m*f(n-m) in O(m^3log(n)) time
	 *
	 * @param coefficients - contains the coefficients for the recurrence, so [c_1,
	 *                     c_2, c_3, ... , c_m]
	 * @param f_0          - The value of the function at f(0). This is usually 1 or
	 *                     k.
	 * @param k            - The constant k added to the recurrence
	 * @param n            - The nth term of the recurrence you wish to find
	 *                     <p>
	 *                     NOTE1: The numbers produced by this method can get VERY
	 *                     LARGE quickly so watch out for overflow because there is
	 *                     a very high probability it will occur.
	 *                     <p>
	 *                     NOTE2: Make sure the dimension of coefficients array is
	 *                     no bigger than it needs to be. For instance do not add
	 *                     any additional zero coefficient terms at the end of the
	 *                     coefficients array as this is throw off the recurrence.
	 *                     <p>
	 *                     EXAMPLE: If your recurrence is f(n) = 2 + 3*f(n-1) +
	 *                     5f(n-4) with f(0) = 2 and you want to find f(100) call
	 *                     the function like: solveRecurrence([3, 0, 0, 5], 2, 2,
	 *                     100)
	 */
	static long solveRecurrence(long[] coefficients, long f_0, long k, long n) {

		if (n < 0)
			throw new IllegalArgumentException("n should probably be >= 0");
		...
		// Add 1 to account for the extra constant 'k' in the recurrence: f(n) = k +
		// c_1*f(n-1) + ...
	 ...
	 *
	 * <p>
	 * Given the constants [c_1, c_2, c_3, ...] and the constant 'k' in the
	 * recurrence f(n) = k + c_1*f(n-1) + c_2*f(n-2) + ... + c_m*f(n-m) this
	 * function computes and returns the initial values of the function: [f(0),
	 * f(1), f(2), ...]
	 *
